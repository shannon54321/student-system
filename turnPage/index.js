(function () {
    ...
    TurnPage.prototype.fillHTML = function () {
        ...
        // 添加第一页按钮
        $('<li class="num">1</li>').appendTo(oUl).addClass(this.curPage == 1 ? 'current-page':'');
        // 添加前面的省略号
        if (this.curPage > 4) {
        ...
        for (var i = this.curPage - 2; i <= this.curPage + 2; i++) {
            if (i > 1 && i < this.allPage && i == this.curPage) {
        ...
        // 添加最后一页
        if (this.allPage > 1) {
              $('<li class="num">' + this.allPage + '</li>').appendTo(oUl).addClass(this.curPage == this.allPage ? 'current-page':'');;
        }
